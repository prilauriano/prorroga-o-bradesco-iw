...
if arquivo_faturamento and arquivo_bradesco:
    try:
        ...
        with aba2:
            ...
            st.dataframe(
                df_view_export.style.format({
                    'Valor Orçado': 'R$ {:,.2f}',
                    'Valor Faturado': 'R$ {:,.2f}',
                    'Data Início': lambda d: d.strftime('%d/%m/%Y') if pd.notna(d) else '',
                    'Data Fim': lambda d: d.strftime('%d/%m/%Y') if pd.notna(d) else '',
                }),
